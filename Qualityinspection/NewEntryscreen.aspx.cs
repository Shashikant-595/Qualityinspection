...
{
    public partial class NewEntryscreen : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 1; i <= 52; i++)
            {
                string controlID = "R" + i;
                TextBox textBox = FindControlRecursive(Page, controlID) as TextBox;
    ...
    r50 == 0 && r51 == 0)
                {
                    ScriptManager.RegisterStartupScript(this,this.GetType(),"SweetAlert", "showSweetAlert('Epty Entry', 'Empty entries are prohibited.', 'Assets/thumb-down.png');", true);
                ...
                else
                {
                    ...
                    {
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "SweetAlert", "showSweetAlert('Oops', 'Entry without Accepted quantity ', 'Assets/thumb-down.png');", true);
            ...
            catch (SqlException ex)
            {
                ...
                ScriptManager.RegisterStartupScript(this, this.GetType(), "SweetAlert", "showSweetAlert('Oops', 'SQL server error occure ', ''Assets/thumb-down.png'');", true);
            }catch (Exception ex)
            {

                ScriptManager.RegisterStartupScript(this, this.GetType(), "SweetAlert", "showSweetAlert('Oops', 'check internet connection  ', ''Assets/thumb-down.png'');", true);
        ...
        protected void DropDownList1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                System.Diagnostics.Trace.WriteLine("list count is  " + codes.Count);
                System.Diagnostics.Trace.WriteLine("list of part no is  " + partno);

                for (int i = 0; i <= codes.Count; i++)
                {

                    string code = codes[i];
                    System.Diagnostics.Trace.WriteLine("event is done " + code[1]);
                    TextBox textBox = FindControlRecursive(Page, code) as TextBox;
                    if (textBox != null)
                    {
